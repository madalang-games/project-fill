# UI/UX — 씬 구조 (Scene Structure)

## 씬 그래프 (Scene Graph)

```
Boot (부트)
  └─(인증 완료)──► Lobby (로비)
                     └─(스테이지 선택)──► InGame (인게임)
                                        └─(오버레이는 인게임 씬 내에 유지)
```

## 씬 목록

| 씬 | 역할 | 진입 시점 | 퇴장 시점 |
|-------|------|-------|------|
| Boot | 인증 체크, 애셋 로드 | 앱 실행 시 | 자동 → 로비 이동 |
| Lobby | 홈/상점/랭킹 탭 관리 | 부트 완료 후 | — (루트 씬) |
| InGame | Signal Sort 보드 게임플레이 + 결과 오버레이 + Stuck 팝업 | 로비에서 스테이지 선택 시 | 로비로 복귀 시 |

## 화면 전환 (Transitions)

| 출발지 | 도착지 | 유형 | 트리거 |
|------|----|------|---------|
| Boot | Lobby | 페이드 (Fade) | 인증 해결 (게스트 또는 정식 계정) |
| Lobby | InGame | 위로 슬라이드 | 스테이지 정보 팝업의 [플레이] 버튼 |
| InGame | Lobby | 아래로 슬라이드 | 결과창의 [로비] 또는 일시정지의 [스테이지 선택] |
| InGame | InGame | 보드 재로드 | 결과창 또는 일시정지의 [재도전] |

## 인게임 오버레이 (씬 전환 없음)

| 오버레이 | 트리거 | 닫기 방법 |
|---------|---------|---------|
| ResultOverlay (결과) | 스테이지 클리어 | [다음] / [재도전] / [로비] 버튼 |
| StuckPopup (교착 상태) | 더 이상 유효 이동이 없는 Stuck 감지 | [광고 시청: 레인 추가] / [셔플] / [포기] 버튼 |
| PausePopup (일시정지) | 일시정지 버튼 탭 | [재개] / [처음부터] / [스테이지 선택] |

## 로비 탭 구조

하단 네비게이션 바를 통해 전환합니다.

| 탭 | MVP 범위 | 2단계 계획 |
|-----|-----|---------|
| 홈 (Home) | 챕터/스테이지 스크롤 | 이벤트 배너 추가 |
| 상점 (Shop) | 아이템 목록 플레이스홀더 | 전체 경제 UI 및 구매 기능 |
| 랭킹 (Ranking) | 글로벌 이동 횟수 랭킹 | 친구 랭킹 추가 |
