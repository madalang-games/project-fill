...
코스메틱 시스템은 게임 플레이에 영향 없는 순수 외관 커스터마이징을 제공합니다. 스테이지 100개 완주 이후에도 플레이어가 목표를 가질 수 있는 **장기 골드 소비처**이자, 데일리 챌린지·업적·출석 시스템의 희귀 보상 채널입니다.

> **설계 원칙**: 코스메틱은 Pay-to-Win 요소 없음. 골드(소프트 커런시)로만 구매 가능하거나 업적/챌린지로 무료 획득. IAP로 코스메틱을 직접 판매하지 않음.
...
| `chip_daily` | Daily Mark | 챌린지 전용 마크 | **챌린지 30일 스트릭** | — |
...
| `board_challenge` | Signal Champion | 챌린지 100일 스트릭 | **챌린지 100일 스트릭** | — |
...
| 항목 | 총 골드 가치 |
| :--- | :---: |
| Chip Skin 구매 가능 총합 | 6,100 골드 |
| Lane Skin 구매 가능 총합 | 7,600 골드 |
| Board Skin 구매 가능 총합 | 10,500 골드 |
| **총 구매 가능 골드 소비** | **24,200 골드** |

100스테이지 완주 플레이어 순 잔액(~7,100 골드 추산, `economy-system-design.md` §4 기준)으로 Chip Skin 전체 + Lane 일부 구매 가능. 데일리 시스템 참여 시 수개월 안에 풀 컬렉션 달성 가능 → 장기 목표 제공.
...
- 상점(Shop) 탭 레이아웃: IAP 패키지 → 골드 구매 → **코스메틱** 순서.
- 코스메틱 섹션: 카테고리 탭 (Chip / Lane / Board) + 그리드 레이아웃.
