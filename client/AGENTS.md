...
- **Sprite/icon refs are NOT static prefab data — never grep-and-edit `m_Sprite`**: A UI Image's sprite is assigned by one of two real sources, never by hand-editing the prefab YAML:
  - **`UIEditorSetup.cs`** via a `dynamic_resource.csv` key (`resMap.TryGetValue("<key>", ...)`) or a serialized `_*ResourceKey` field default. To change it, edit the key in `UIEditorSetup.cs` (then user re-runs the UI menu).
  - **Per-instance inspector override** in a `*.unity` scene or Final variant (e.g. `BoardView` `_skin.*` on the InGameCanvas instance). To change it, re-wire the field on that instance — directly editing the scene's `objectReference` GUID is the only in-repo path; otherwise FLAG the user to set it in the Unity inspector.
  - **Base prefabs (`Assets/UI/Prefabs/Base/**`) are throwaway** — regenerated by `UIEditorSetup.cs`, and a Final-variant / scene-instance override outranks them at runtime. Editing a Base prefab's `m_Sprite` is meaningless.
