...
| `RewardDisplay.cs` | `RewardDisplay` | Static helper; `Build(GrantedRewardDto[])` (claim results) + `BuildFromGroup(rewardGroupId)` (static reward_group preview, sort_order-ordered) → `RewardPopupView.RewardItem[]`. `ResolveVisual(rewardType,targetId)` is the single source for reward icon-key + name/desc lookup (gold via `currency.icon_name`, item via `item.icon_name`, avatar, no-ads) — reused by `ShopTabView` preview rows. Shared by attendance/achievement/challenge/chest flows |
...
| `AttendancePopupView.cs` | `AttendancePopupView` | Daily attendance popup: 7 day cards (claimed/today/future) each stacking ALL of that day's reward-group items vertically as runtime `RewardItemCell`s (long-press tooltip) in `RewardSlot` (no LayoutGroup — cells scaled via `localScale` to fit, manual stack), + `TodayRewardRow` (HLG) showing today's full reward group, claim CTA → RewardPopup; rewards resolved via `RewardDisplay.BuildFromGroup`; auto-opened from `LobbyView` |
...
| `WeeklyMissionPopupView.cs` | `WeeklyMissionPopupView` | Weekly Mission Event popup: 5-mission checklist + EP gauge + reward-track milestone summary + claim CTA (lowest reached-unclaimed milestone); reads `WeeklyMissionApiService`, reward display via `RewardDisplay` (design §6.2) |
...
| `StageInfoPopupView.Init(stageId,bestMoves,onPlay,difficulty,isLocked)` | method | Required before showing; bestMoves 0 → "-"; isLocked=true keeps PLAY interactable but `OnPlay` surfaces `toast.stage_locked` instead of entering (no dead button); difficulty tints ribbon: 0=amber(default), 1=teal, 2=crimson. Queries `StageDataService.GetStage` for difficulty label + signal-type count + gimmick badge toggles (overload/relay/locked-lane/blind-lane) |
...
| `CosmeticSectionView.Rebuild()` | method | Builds grid for current `CosmeticCategory`; preview built by `CosmeticPreview.Build` (layered procedural skin); toggles `Preview/SelectedHighlight` (IsActive) + `Preview/LockOverlay` (!Unlocked) per cell; cell tap → CosmeticPreviewPopupView |
