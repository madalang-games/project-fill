...
namespace Game.InGame.View
{
    ...
    public class SignalNodeView : MonoBehaviour
    {
        ...
        private void Construct(SpriteSet s, bool showConnector, Vector2 size)
        {
            ...
            // Circular disc body + concentric ring (off-state colors; Update lerps toward targets).
            _body = WorldUtil.CreateSprite(transform, "Body", s.Disc, OffBody, size, sliced: false, sortingOrder: 4);
            _ring = WorldUtil.CreateSprite(transform, "Ring", s.Ring, OffRing, size, sliced: false, sortingOrder: 5);
        ...
        public void SetState(bool lit, bool pending)
        {
            ...
            if (lit)
            {
                // Body/ring base mixes; Update shimmers the white-blend within this color's range.
                ...
                // competing with the celebratory color shimmer.
        ...
        private void Update()
        {
            ...
            if (Lit)
            {
                // Completed shimmer: oscillate the white-blend within this signal's color range so the
                // lit core "breathes" bright→white-hot, reading as a stronger finished/celebratory state.
                float shimmer = Mathf.Sin(Time.unscaledTime * 3f) * 0.5f + 0.5f; // 0..1
                bodyTarget = Color.Lerp(_glowColor, Color.white, Mathf.Lerp(0.22f, 0.50f, shimmer));
                ringTarget = Color.Lerp(_glowColor, Color.white, Mathf.Lerp(0.70f, 0.95f, shimmer));
            }
            ...
            // Glow: breathing when lit, faint when pending, off otherwise — plus the register burst.
            _flash = Mathf.MoveTowards(_flash, 0f, Time.deltaTime * 1.4f);
            float pulse = Mathf.Sin(Time.unscaledTime * 4f) * 0.5f + 0.5f;
            float baseA = Lit ? Mathf.Lerp(0.18f, 0.38f, pulse) : (_pending ? 0.06f : 0f);
