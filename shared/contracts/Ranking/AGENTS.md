...
| `RankingEntryDto.Score` | property | Max cleared stage (`stage`), perfect-clear count (`perfect`), or weekly cleared count (`weekly`) depending on ranking type |
...
- Global ranking types: `stage` (max cleared stage), `perfect` (perfect-clear count). Weekly ranking reuses `RankingPageResponse`/`MyRankingResponse` with `RankingType = "weekly"`.
