...
| symbol | kind | note |
|--------|------|------|
| `StageService.StartStageAsync` | method | Stage-entry gate: validates stage exists + reachable (`ValidateUnlockedAsync`); returns `StageStartResponse` (server-authoritative `max_cleared_stage_id` + ruleset). Sessionless — no attempt token |
...
| `StageService.ClearStageAsync` | method | Records a stage clear; rejects locked stages via `ValidateUnlockedAsync`; grants first-clear reward + chapter chest milestone; updates `user_ranking_totals`/`user_weekly_ranking`; syncs Redis via `RankingService.RecordClearAsync`; returns `StageClearResponse` |
...
## Rules
- Stage entry is server-gated: `StartStageAsync` (sessionless) validates the stage is reachable before the client builds the board. `ClearStageAsync` re-runs the same `ValidateUnlockedAsync` so the gate cannot be bypassed by posting a clear directly. Unlock rule: stage 1 open; stage N requires `max_cleared_stage_id >= N-1` (`StageLocked` otherwise).
