    ...
    public sealed class IapService
    {
            ...
            CancellationToken ct)
        {
            ...
            _db.EventLogs.Insert(EventLogFactory.IapPurchaseCompleted(
                userId, correlationId, product.Id, product.StoreProductId,
            ...
            int? remaining = GetRemainingPurchases(product, await GetPurchaseCountAsync(userId, product.Id, ct));
        ...
        private async Task ValidatePurchaseLimitAsync(long userId, IapProductData product, CancellationToken ct)
        {
            ...
            var row = await _db.UserIapPurchaseCounts.FindAsync(userId, product.Id, ct);
