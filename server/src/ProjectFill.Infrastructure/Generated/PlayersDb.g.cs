...
internal sealed class PlayersDbConfiguration : IEntityTypeConfiguration<PlayersRow>
{
    public void Configure(EntityTypeBuilder<PlayersRow> builder)
    {
        ...
        builder.Property(e => e.ConflictCloudPid)
            ...
            .HasColumnType("VARCHAR(24)");
        builder.Property(e => e.IsNoAds)
            .HasColumnName(PlayersDb.Schema.IsNoAds)
...
public sealed class PlayersDb
{
    public static class Schema
    {
        ...
        public const string IsNoAds = "is_no_ads";
    }
    ...
    public Task InsertIgnoreAsync(PlayersRow row, CancellationToken ct = default)
        => _db.Database.ExecuteSqlInterpolatedAsync(
            $"INSERT IGNORE INTO `players` (`user_id`, `platform_pid`, `display_name`, `avatar_id`, `equipped_board_theme_id`, `account_created_at`, `last_login_at`, `is_active`, `conflict_cloud_pid`, `is_no_ads`) VALUES ({row.UserId}, {row.PlatformPid}, {row.DisplayName}, {row.AvatarId}, {row.EquippedBoardThemeId}, {row.AccountCreatedAt.UtcDateTime}, {row.LastLoginAt.UtcDateTime}, {(row.IsActive ? 1 : 0)}, {row.ConflictCloudPid}, {(row.IsNoAds ? 1 : 0)})",
            ct);
    ...
    public IQueryable<(PlayersRow Player, IapPurchasesRow IapPurchase)> JoinWithIapPurchases()
        => Query().Join(_db._IapPurchases, l => l.UserId, r => r.UserId,
            (l, r) => ValueTuple.Create(l, r));
